...
$(function() {
    ...
    d3.select("#slider").on("input", function () {
        ...
        if (value === 0) {
            ...
            barchart = false;
            tableContainer.style.display = "none";
            const themes = nodes.filter(d => d.group === "thema");
            const objects = nodes.filter(d => d.group !== "thema");
            bubblechart(objects,themes, svg2);
    ...
    function bubblechart(objects, themes, svg2) {
        // Erstelle eine Gruppierung der Objekte nach ihrer 'group' mit d3.group()
        const groupedNodes = d3.group(objects, d => d.group);
    
        nodeGroup.transition().duration(2000)
            .attr("transform", function(d) {
                if (d.group !== "thema") {
                    // Finde die Gruppe des aktuellen Knotens
                    const group = groupedNodes.get(d.group); // Holen der Gruppe aus dem Map-Objekt
                    const groupIndex = group ? group.indexOf(d) : 0;
    
                    // Berechne die Position für diese Gruppe (Hier einfach eine Position pro Gruppe, modifizieren je nach Bedarf)
                    const offsetX = 2 * groupIndex; // Offset für jede Gruppe, kann angepasst werden
                    const offsetY = 2 * Math.floor(groupIndex / 5); // Vertikale Gruppierung der Nodes, je nach Anzahl der Gruppen
    
                    // koordinaten des Knotens berechnen
                    const [lat, lon] = d.koordinaten.split(", ").map(Number);
                    const [x, y] = projection([lon, lat]);
    
                    // Kombiniere die Projektionskoordinaten mit dem Gruppierungsoffset
                    return `translate(${x + offsetX},${y + offsetY})`;
                }
                return d3.select(this).attr("transform"); // "thema"-Knoten bleiben an ihrem Platz
            });
    }
